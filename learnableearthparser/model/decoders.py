...
class Decoders:
    ...
    def transform_translate(self, params, protos):
        if self.use_gt:
            gt_pos = self.gt_pos.to(params.dtype)
            gt_pos[...,:2] += gt_pos[...,3:5] / 2.
            gt_pos[..., 2] += gt_pos[...,5] / 2.
            offset = params.size(1)-gt_pos.size(1)
            gt_padded = torch.cat((gt_pos, gt_pos[:,:offset,:]), dim=1)[...,:3].unsqueeze(-2).unsqueeze(-2)
            gt_padded[...,:2] -= self.hparams.data.max_xy / 2.
            gt_padded[..., 2] -= self.hparams.data.max_z / 2.
            return protos + gt_padded, params
        else:
