...
class BaseModel(pl.LightningModule, LoggingModel, Decoders, Prototypes):
    ...
    def do_step(self, batch, batch_idx, tag):
        ...
        with torch.no_grad():
            with torch.profiler.record_function(f"LOGGERS"):
                ...
                if "chamfer_masked" in out.keys() and out["chamfer_masked"] is not None:
                    self.log(
                        f'Losses/chamfer_masked/{tag}', out["chamfer_masked"].mean(), on_step=False, on_epoch=True, batch_size=out["batch_size"])
                
                if "chamfer_x" in out.keys() and out["chamfer_x"] is not None:
                    self.log(
                        f'Losses/chamfer_x/{tag}', out["chamfer_x"].mean(), on_step=False, on_epoch=True, batch_size=out["batch_size"])
                           
                if "chamfer_y" in out.keys() and out["chamfer_y"] is not None:
                    self.log(
                        f'Losses/chamfer_y/{tag}', out["chamfer_y"].mean(), on_step=False, on_epoch=True, batch_size=out["batch_size"])
                
                if "chamfer_y_masked" in out.keys() and out["chamfer_y_masked"] is not None:
                    self.log(
                        f'Losses/chamfer_y_masked/{tag}', out["chamfer_y_masked"].mean(), on_step=False, on_epoch=True, batch_size=out["batch_size"])
                ...
                for l in ["l_XP", "l_PX", "l_gamma", "l_gamma0", "l_KL_K", "l_KL_L", "l_Lmean", "l_Kmean", "l_entropie_K", "l_entropie_L", "l_freq_K", "l_freq_L", "l_xytranslate", "l_overlap"]:
                    if l in out.keys():
                        self.log(
    ...
    def forward(self, batch, tag, batch_size, batch_idx):
        ...
        # If use_gt is True, force gt positions of prototypes
        use_gt = getattr(self.hparams, "use_gt", False)
        if use_gt:
            protos, out = self.get_transformed_protos(encoded, gt=batch.gt)
        else:
    ...
    def compute_reconstruction_loss(self, tag, batch, batch_size, out, protos, proto_slab, bkg=None, batch_idx=None):
        if "recs" not in out.keys():
            ...
            # Compute the additional mask for occlusion
            mask_y_soft = out["masks"]
            mask_occl = torch.zeros_like(mask)
            for s in range(proto_slab.size(0)):
                rec_mask = torch.zeros(max_y_lenght, device=mask.device)
                rec_mask = rec_mask.view(-1, self.hparams.protos.points)
                i = 0
                for l, k in enumerate(out["choice"][s]):
                    if k != -1:
                        rec_mask[i] = mask_y_soft[s, l, k]
                        i+=1
                mask_occl[s] = rec_mask.flatten()
            mask_comb = mask * mask_occl
            # Log additional chamfer values
            out["chamfer_masked"] = .5 * (cham_x.sum(-1) / batch.pos_lenght + (mask_comb * cham_y).sum(-1) / mask_comb.sum(-1))
            out["chamfer_x"] = cham_x.sum(-1) / batch.pos_lenght
            out["chamfer_y"] = (mask * cham_y).sum(-1) / mask.sum(-1)
            out["chamfer_y_masked"] = (mask_comb * cham_y).sum(-1) / mask_comb.sum(-1)
    ...
    def configure_optimizers(self):
        ...
        if hasattr(self, "_protos"):
            parameters.append({
                "params": self._protos if self.hparams.protos.name in ["points","mesh"] else self._protos.parameters(), "name": "protos",
