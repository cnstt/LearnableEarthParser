...
class OutHTML(DTICallback):
    ...
    def get_protos(self, protos):
        ...
        # Figure update layout somehow only applied for first subplot
        # So figure.update_scenes is used instead
    ...
    def get_inferences(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule"):
        ...
        # Figure update layout somehow only applied for first subplot
        figure.update_scenes(aspectmode='data') # , aspectratio=dict(x=1, y=1, z=1)
        figure.update_layout(legend=dict(
            orientation="h",
            yanchor="bottom",
            y=1.,
