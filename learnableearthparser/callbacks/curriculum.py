...
class Curriculum(DTICallback):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if hasattr(self.myhparams, 'patience_eps'):
            self.patience_eps = self.myhparams.patience_eps
        else:
            self.patience_eps = 0
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        if (len(self.order) != 0) and (self.to_activate is None):
            ...
            elif self.mode*trainer.callback_metrics[self.monitor].item() > self.mode*(self.last_value+self.mode*self.patience_eps):
                self.last_value = trainer.callback_metrics[self.monitor].item()
                self.count = 0
            else:
                self.count += 1
